...
from generalUtils.mp_utils import MPFileHandler, MPStreamHandler
logging.basicConfig(filename='logs/test_handlers.log', level=logging.DEBUG, filemode='w')


def test_file_usage():
    filename = 'logs/mplogger_file_handler.log'
...
def test_multiprocess_file():
    filename = 'logs/mplogger_mp_file.log'
    test_l = 100

    def worker(wait, file):
        import logging
        from generalUtils.mp_utils import MPFileHandler
...
def test_multiprocess_stream(capfd):
    filename = 'logs/mplogger_mp_stream.log'
    test_l = 100

    def worker(wait, file):
        import logging
        from generalUtils.mp_utils import MPStreamHandler
        logger = logging.getLogger('mp'+str(wait))
