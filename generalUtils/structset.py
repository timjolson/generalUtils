...
class StructSet(SimpleNamespace):
    ...
    from the object.
    StructSet is also subcriptable, and supports basic +/-, +=/-=, ==, !=.
    ...
    obj = StructSet(a=2, b=[3,4], c='hello')
    *OR
    obj = StructSet({'a':2, 'b':[3,4], 'c':'hello'})
    ...
    def __init__(self, *arg, **kwargs):
        if arg and not kwargs:
            if len(arg) == 1 and type(arg[0]) == dict().__class__:
                ...
                raise Exception(
                    'StructSet() takes either a dict or keyword arguments. {} provided'.format(type(arg[0])))
